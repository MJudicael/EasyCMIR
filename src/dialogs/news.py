...
class NewsDialog(QDialog):
    ...
    def load_asn_news(self):
        try:
            ...
            page = requests.get("https://www.asn.fr/l-asn-informe/actualites")
            soup = BeautifulSoup(page.content, 'html.parser')
            
            # On cherche les articles qui contiennent à la fois le titre et le lien
            news_items = soup.find_all("article", class_="Teaser")
            
            # Formatage des news en HTML avec liens
            news_html = "<h2>ACTUALITÉS ASN:</h2>"
            
            for item in news_items:
                # Récupère le lien et le titre
                link_element = item.find("a", class_="Teaser-titleLink")
                if link_element:
                    title = link_element.get_text().strip()
                    # Le lien est relatif, on ajoute donc le domaine
                    link = "https://www.asn.fr" + link_element.get('href', '')
